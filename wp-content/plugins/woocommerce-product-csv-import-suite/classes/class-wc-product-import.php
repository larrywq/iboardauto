	...
	function dispatch() {
		...
		switch ( $step ) {
				...
				if ( $file ) {
					...
					<script type="text/javascript">
						jQuery(document).ready(function($) {
							...
							var processed_terms = [ 0 ];
							var processed_posts = [ 0 ];
							var post_orphans    = [ 0 ];
							var url_remap       = [ 0 ];
							var i               = 1;
							var done_count      = 0;
							...
							if ( ( $handle = fopen( $file, "r" ) ) !== FALSE ) {
								...
								while ( ( $postmeta = fgetcsv( $handle, 0, $this->delimiter ) ) !== FALSE ) {
						            ...
						            if ( $count >= $limit ) {
										...
										$import_count      ++;
						            	?>
						            	import_rows( <?php echo $previous_position; ?>, <?php echo $position; ?> ); // Import rows between <?php echo $previous_position; ?> and <?php echo $position; ?>
			...
			case 3 :
				...
				$mapping   = json_decode( $_POST['mapping'] );
				$start_pos = isset( $_POST['start_pos'] ) ? absint( $_POST['start_pos'] ) : 0;
				$end_pos   = isset( $_POST['start_pos'] ) ? absint( $_POST['end_pos'] ) : '';

				if ( ! is_file( $file ) ) {
					echo json_encode( array( 'error' => __( 'File not found - aborting', 'wc_csv_import' ) ) );
				}
			...
			case 4 :
				...
				$this->processed_terms = $_POST['processed_terms'];
				$this->processed_posts = $_POST['processed_posts'];
				$this->post_orphans    = $_POST['post_orphans'];
				$this->url_remap       = $_POST['url_remap'];
	...
	function import() {
		...
		foreach ( $this->parsed_data as $key => &$item ) {
			...
			if ( ! is_wp_error( $product ) )
				...
				$this->add_import_result( 'failed', $product->get_error_message(), '-', 'Not parsed', '-' );
